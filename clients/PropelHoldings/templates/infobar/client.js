(function () {
	...
	function setInfobarPosition(context) {
		if (context.infobarClass === 'evg-infobar-top') {
			SalesforceInteractions.cashDom('main.x-layout__main div.x-user-select-none--nested').css({ 'margin-bottom': '0', 'margin-top': '2.5rem' });
		} else {
			SalesforceInteractions.cashDom('main.x-layout__main div.x-user-select-none--nested').css({ 'margin-bottom': '2.5rem', 'margin-top': '0' });
		}
	}
	...
	function setDismissal(context) {
		SalesforceInteractions.cashDom(`#evg-infobar-with-cta.${context.infobarClass} .evg-btn-dismissal`).on('click', () => {
			...
			SalesforceInteractions.cashDom('main.x-layout__main div.x-user-select-none--nested').css({ 'margin-top': '0', 'margin-bottom': '0' });
		});
	}
	...
	function apply(context, template) {
		...
		SalesforceInteractions.cashDom('main.x-layout__main div.x-user-select-none--nested').before(html);
